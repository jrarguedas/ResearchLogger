...
class DistributionBuilder:
    ...
    def toggle_stealth(self, new_name, new_description, icon_flag):
        '''
        Change name, description, in version.py, rename the icon files and the
        ini and val files.
        '''
        f = open('version.py', 'r')
        try:
        ...
        if icon_flag == 1:
            ...
            shutil.copy(version.name + 'icon_big.gif', new_name
                        + 'icon_big.gif')
        else:
    ...
    def build_nsis_installer(self):
        '''
        This needs to be called before we zip and delete the renamed dist
        directory.
        '''
    ...
    def update_nsis_script_version(self):
        ...
        try:
            for line in contents:
                line = re.sub('^( *!define PYKEYLOGGER_VERSION ).*', '\\1'
